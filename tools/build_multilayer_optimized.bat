...
REM PyQt6 版本 - Genshin Impact 风格界面
...
if errorlevel 1 (
    ...
    if errorlevel 1 (
        echo [ERROR] PyQt6 安装失败！
        pause
        exit /b 1
    )
)
python -c "import qtawesome" >nul 2>&1
if errorlevel 1 (
...
REM 使用 PyInstaller 构建（优化版 PyQt6）
REM 关键变化：
REM   1. --onedir (替代 --onefile) - 大幅提升启动速度
REM   2. 添加 PyQt6 必要的隐藏导入
REM   3. 添加 --noupx（避免 UPX 压缩带来的启动延迟）
REM   4. 收集 qtawesome 数据文件
...
REM 检查输出目录
set "OUTPUT_DIR=%DIST_DIR%\WatermarkApp_PyQt6_v1.6.2_Optimized"
set "EXE_FILE=%OUTPUT_DIR%\WatermarkApp_PyQt6_v1.6.2_Optimized.exe"

if exist "%EXE_FILE%" (
    echo [OK] 输出目录: %OUTPUT_DIR%
    echo [OK] 主程序: WatermarkApp_PyQt6_v1.6.2_Optimized.exe
    echo.
    ...
    echo   2. 分发给用户后解压使用
    echo   3. 双击运行: WatermarkApp_PyQt6_v1.6.2_Optimized.exe
    echo.
    echo 下一步优化（可选）:
...
if exist "%EXE_FILE%" (
    (
        echo Multi-Layer Watermark App v1.6.2 PyQt6 - 快速开始
        echo ========================================
        echo.
        echo 界面版本:
        echo   PyQt6 版本 - Genshin Impact 原神风格界面
        echo.
        echo 运行方法:
        echo   双击: WatermarkApp_PyQt6_v1.6.2_Optimized.exe
        echo.
        echo 优化说明:
        echo   - 使用 onedir 模式，启动速度提升 5-10 倍
        echo   - 从 5-7 秒降至 0.5-1 秒
        echo   - PyQt6 现代化界面，更流畅的用户体验
        echo.
        echo 分发说明:
        ...
        echo   - 确保所有文件解压到同一目录
        echo   - PyQt6 需要 Windows 10/11（不支持 Windows 7）
        echo.
        echo 文档资源:
