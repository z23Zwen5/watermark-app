...
def main():
    ...
    print(f"创建 QApplication: {(time.time() - t0)*1000:.0f}ms")
    ...
    try:
        ...
        print(f"预加载图标字体: {(time.time() - t0)*1000:.0f}ms")
    except:
        print(f"预加载图标字体: 失败")
    ...
    print(f"应用全局样式: {(time.time() - t0)*1000:.0f}ms")

    # 创建并显示主窗口
    t0 = time.time()
    window = MainWindow()
    print(f"创建主窗口: {(time.time() - t0)*1000:.0f}ms")

    t0 = time.time()
    window.show()
    print(f"显示窗口: {(time.time() - t0)*1000:.0f}ms")
